...
<template>
    ...
    <div class="content">
      <aside class="sidebar">
        <div class="section-select">
          ...
          <div class="section-label">Section {{ selectedSection }}</div>
        </div>
       <ul class="menu">
  ...
  <li :class="{ active: activeMenu === 'Evidencia' }" @click="activeMenu = 'Evidencia'">
    <a href="/evidencia_sub" style="color: inherit; text-decoration: none;">🖼️ Evidencia</a>
  </li>
</ul>
      ...
      <main class="main-content">
        <div class="section-header">
          <span class="section-title">Section {{ selectedSection }}</span>
        </div>
          ...
          <table>
            ...
            <tbody>
              <tr v-for="(est, i) in estudiantes" :key="i">
                ...
                <td>{{ est.actividad }}</td>
                <td>{{ est.horas }}</td>
                <td><button class="btn-evidencia">Ver Evidencia</button></td>
                <td>
                  <span class="estado" :class="est.estado === 'Pendiente' ? 'pendiente' : 'aprobado'">
                    {{ est.estado }}
                  </span>
                </td>
                <td>
                  <button class="btn-aprobar" :class="{ aprobado: est.estado === 'Aprobado' }" @click="aprobar(est)">
                    {{ est.estado === 'Aprobado' ? 'Aprobado' : 'Aprobar' }} <span class="arrow">▼</span>
                  </button>
                </td>
              </tr>
            </tbody>
...
.section-select select { width: 100%; padding: 0.3rem 0.5rem; border-radius: 8px; border: 1px solid #ccc; margin-bottom: 0.5rem; font-size: 1rem; }
